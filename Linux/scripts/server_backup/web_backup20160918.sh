...
mkdir -p /backup/$IP
cd / &&\
tar zcfh /backup/$IP/backup-$Time.tar.gz etc/sudoers etc/ssh/sshd_config var/log/messages etc/hosts etc/sysconfig/iptables server/scripts etc/rc.local var/spool/cron/root var/html/www app/logs && \
