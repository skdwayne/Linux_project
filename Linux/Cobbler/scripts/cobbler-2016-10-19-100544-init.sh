...
yum install kmod-drbd84 drbd84 heartbeat -y
modprobe drbd
wget http://10.0.0.132/cobbler/ks_mirror/config/ha.d.tar.gz
wget http://10.0.0.132/cobbler/ks_mirror/config/drbd.d.tar.gz
tar xf ha.d.tar.gz -C /etc
chmod 600 /etc/ha.d/authkeys
tar xf drbd.d.tar.gz -C /etc &&\
drbdadm create-md drbd

[ $(hostname) = "nfs01" ] && drbdadm -- --overwrite-data-of-peer primary drbd && mkfs.ext4 /dev/drbd0
...
#    hostname_config
    ...
    zabbix_agent_config
    salt_minion_config
    [ $(hostname) = "nfs01" -o $(hostname) = "backup" ]  &&  drbd_nfs_config
